...
import { POPULAR, F2P_FALLBACK, MINI_FALLBACK, OPTIMAL_FALLBACK } from "@/lib/questLists";
...
export default function QuestHelper() {
  ...
  // Deep link support (?quest=Name), e.g. from the dashboard's "next
  // quest" card — plain window.location instead of useSearchParams so
  // this page can stay statically rendered.
  useEffect(() => {
    const name = new URLSearchParams(window.location.search).get("quest");
    if (name) openQuest(name);
